...
- **Fase 1 — P1 (antes de la próxima versión menor; alto impacto / bajo esfuerzo):** `S3-03` (arranque del daemon: bind del puerto), `S3-06` (oferta de fuente GPL en el release), `S3-07` (smoke test del binario congelado). (Ya resueltos y fuera de esta fase: `S3-01` —contrato de cancelación, cerrado el 2026-07-12: el handler de `main()` en `cli.py:1661` ya devuelve `EXIT_INTERRUPTED` (130)—; `S3-02` —carrera del doble arranque, cerrada el 2026-07-12 con un PID/lock file atómico—; `S2-03` —documentar `--force-update` en `USAGE.md`, cerrado el 2026-07-12.)
