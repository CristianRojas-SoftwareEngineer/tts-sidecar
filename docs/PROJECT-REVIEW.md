...
Conteo por severidad: **0 S4, 2 S3, 18 S2, 18 S1, 5 S0** (43 hallazgos consolidados). Dos afirmaciones de alta severidad propuestas por los sub-agentes fueron **descartadas como falso positivo** tras verificación directa; además, la presunta discrepancia de conteo de tests no es defecto (ver «Nota de verificación» y «Provenance»). El conteo en el momento de la auditoría era 350 tests recolectados por pytest (336 funciones `def test_`); las remediaciones posteriores de los hallazgos resueltos (S3-01/02, S2-02, S2-07, S1-17, S2-10, S2-05, S2-04, etc.) elevaron la suite a **506 tests recolectados**. `CLAUDE.md` (~506) refleja el total actual.
...
| S2-12 | `bootstrap` usa `warnings.filterwarnings("ignore")` global | S2 — Medio | P2 | bootstrap / Observabilidad | Sí | Resuelto |
...
- **Estado**: Resuelto
- **Remediación (opción A + `_synthesis_timing`, lote cluster de globals)**: los tres globals `_engine`/`_server`/`_start_time` y sus setters módulo-level (`set_engine`/`set_server`/`set_start_time`) se eliminaron. En su lugar, un dataclass **`DaemonState`** (`engine`/`server`/`start_time`) vive en **`app.state.daemon`** y los endpoints lo reciben por inyección: `Depends(get_daemon_state)`, donde `get_daemon_state(request)` devuelve `request.app.state.daemon`. `/health`, `/synthesize`, `/voices` y `/shutdown` leen el estado inyectado (p. ej. `/shutdown` libera `state.engine = None`). El **composition root** es `daemon/run.py`, que construye un `DaemonState` fresco por iteración de `serve()` (relevante en `--auto-restart`) y lo puebla conforme crea el engine y el `uvicorn.Server`; ya no hay setters. No se cayó en la trampa del parche: el estado no es un global mutable de módulo sino estado del objeto `app`, sustituible por test vía `app.dependency_overrides[get_daemon_state]` (nueva clase `tests/test_daemon.py::TestDaemonStateInjection`, 3 tests). **`_synthesis_timing`** (alcance aprobado): el engine dejó de exponer un dict suelto y ahora publica **`SynthesisMetrics`** (dataclass tipada `t3`/`s3gen` en `timing.py`); `server.py` lo consume como contrato tipado (`metrics.t3`/`metrics.s3gen`) en vez de `getattr(engine, "_synthesis_timing", {})`, rompiendo el acoplamiento por dict-vía-globals que el deep-dive señalaba. Los ~15 tests del daemon que mutaban `server._engine`/`set_engine` migraron a `server.app.state.daemon.engine`; los fakes que fijaban `_synthesis_timing` usan `SynthesisMetrics`. La opción B (factory `create_app`) se descartó por su costo de migración masiva de `TestClient(server.app)`; la opción A logra el desacople real (endpoints sin globals, testeable por inyección) con superficie acotada.
...
- **Corrección(es) propuesta(s)**: Ver «Alternativas y trade-offs».
- **Decisión requerida**: Sí — **aprobada la opción C** (allow-list explícita documentada, sin catch-all). Se eliminó el `warnings.filterwarnings("ignore")` global y el `os.environ["PYTHONWARNINGS"]="ignore"`, y se conservó el filtro específico de `pkg_resources` dentro de la allow-list; se confirmó que ningún flujo depende del silencio total (no hay código que decida a partir de salida limpia).
- **Estado**: Resuelto
- **Resolución (2026-07-13)**: se eliminó el catch-all `warnings.filterwarnings("ignore")` (`bootstrap.py:83`) y la línea `os.environ["PYTHONWARNINGS"] = "ignore"` (`bootstrap.py:88`), sustituyéndolos por una **allow-list explícita** `_SILENCED_WARNINGS` en `src/tts_sidecar/bootstrap.py` que silencia únicamente dos warnings benignos de terceros: `("pkg_resources is deprecated", Warning, None)` (lo emite `perth` al importar `pkg_resources` en Python 3.13; `category=Warning` en vez de `DeprecationWarning` porque `perth` lo emite como `UserWarning` en este entorno, y con `Warning` queda acotado por mensaje y cubre ambas categorías) y `(None, DeprecationWarning, r"^diffusers\.")` (warning de `LoRACompatibleLinear` al importar `chatterbox`, filtrado por módulo para no atarse al texto exacto). Las tres supresiones de `logging` (`huggingface_hub`, `tokenizer` pkuseg, `alignment_stream_analyzer`) se conservan intactas. La supresión se aplica en un bucle `warnings.filterwarnings("ignore", message=..., category=..., module=...)` por entrada. Se añadieron tests de regresión en `tests/test_cli.py::TestBootstrap`: `test_apply_has_no_catch_all_warning_filter` (no hay entrada catch-all en `warnings.filters`), `test_silenced_warnings_allow_list_has_expected_entries` (la allow-list declara las dos entradas) y `test_apply_does_not_silence_unrelated_warnings` (un `UserWarning` de control propaga), y se actualizó `test_apply_sets_expected_env_vars` para dejar de afirmar `PYTHONWARNINGS`. Suite: 501 passed, 0 warnings. Cierra el hallazgo sin reintroducir ruido enmascarado.
- **Reversión**: restaurar en `bootstrap.apply()` el bloque a `warnings.filterwarnings("ignore")` + `os.environ["PYTHONWARNINGS"] = "ignore"`, conservando el filtro específico original `warnings.filterwarnings("ignore", message="pkg_resources is deprecated", category=DeprecationWarning)`; borrar la constante `_SILENCED_WARNINGS` y el bucle que la aplica; en `tests/test_cli.py` reañadir `"PYTHONWARNINGS"` a `test_apply_sets_expected_env_vars` (con `assert os.environ["PYTHONWARNINGS"] == "ignore"`) y eliminar los tres métodos nuevos de `TestBootstrap`.
- **Prioridad**: P2
- **Alternativas y trade-offs**:
...
- **Estado**: Resuelto
- **Remediación (opción A, lote cluster de globals)**: `bootstrap.apply()` se consolidó como **capa única** de init del proceso absorbiendo la reconfiguración UTF-8 de stdout/stderr (antes vivía solo a nivel de módulo en `cli.py`), como su primera acción, protegida ante streams no reconfigurables. Se **eliminó el `apply()` a nivel de módulo de `cli.py`**: importar `tts_sidecar.cli` ya no tiene el efecto colateral de bootstrapear, así que desaparece el invariante «todo import posterior debe ir tras el apply() implícito». Ahora `apply()` se invoca **explícitamente** en cada entry point como su primera acción: `main()` (vía pip `cli:main` y la congelada `bin/tts-sidecar`→`main`), `bin/tts-sidecar` y `src/tts_sidecar/__main__.py` (ambos antes de importar el CLI, simétricos con `daemon/run.py`, que ya lo hacía). Es idempotente, así que las invocaciones encadenadas son no-op. El contrato de warnings silenciados de CLAUDE.md se preserva **verbatim** (no se tocó la lista de `filterwarnings`/loggers). Nuevos tests: `test_apply_reconfigures_streams_to_utf8` y `test_apply_survives_unreconfigurable_stream` (`test_cli.py::TestBootstrap`).
- **Refinamiento sobre `ensure_path()`**: la opción A sugería además un helper `ensure_path()` que factorizara el ajuste de `sys.path`. La lectura directa confirma que ese ajuste es necesario en **un solo sitio** (`bin/tts-sidecar` en modo fuente) y es **pre-import por naturaleza**: sirve para volver importable el paquete, así que no puede vivir *dentro* del paquete sin un problema de huevo-y-gallina (habría que importar `bootstrap` antes de poner `src/` en el path). `python -m tts_sidecar`, `daemon.run` y el binario congelado ya tienen el paquete importable y no lo necesitan. Factorizarlo a un helper compartido lo dejaría como código muerto (un único llamador que además no podría usarlo pre-import); por eso el shim mínimo se mantiene en `bin/tts-sidecar` —simplificado y documentado como irreducible— en vez de forzar un `ensure_path()` cargo-cult. La unificación real (capa única + invocación explícita consistente) sí se implementó.
